...
class KITTI360Loader:
    ...
    def _load_all_cameras(self, sequence_name):
        ...
        # cam_02_to_world_dict = dict()
        # for frame_id in frame_ids:
        #     imu_to_world = imu_to_world_dict[frame_id]
        #     cam_02_to_world_unrec = imu_to_world @ cam_02_to_imu
        #     cam_02_to_world = cam_02_to_world_unrec @ np.linalg.inv(cam_02_rec)
        #     cam_02_to_world_dict[frame_id] = ct.convert.pad_0001(cam_02_to_world)
        # cam_03_to_world_dict = dict()
        # for frame_id in frame_ids:
        #     imu_to_world = imu_to_world_dict[frame_id]
        #     cam_03_to_world_unrec = imu_to_world @ cam_03_to_imu
        #     cam_03_to_world = cam_03_to_world_unrec @ np.linalg.inv(cam_03_rec)
        #     cam_03_to_world_dict[frame_id] = ct.convert.pad_0001(cam_03_to_world)
        ...
        #save to poses in npy file
        seq_id = 1538
        #cam 00
        cam_00_T_np = np.reshape([cam_00_to_world_dict[frame_id] for frame_id in range(seq_id, seq_id+64)], (-1, 4 , 4)) #[N, 4, 4]
        cam_00_hwf = np.array((cam_00_K[0,2], cam_00_K[1,2], cam_00_K[0,0])) # hight, width, fx [3,]
        cam_00_hwf = np.broadcast_to(cam_00_hwf[:, None], cam_00_T_np.shape[:1] + (3,1)) #[N, 3, 1]
        cam_00_Tk_np = np.concatenate((cam_00_T_np[:, :3, :], cam_00_hwf), axis=2) #[N, 3, 5]
        # np.save(f'{sequence_name}_{seq_id}_cam_00_T_np', cam_00_Tk_np..reshape(-1, 3*5))
        #cam 01
        cam_01_T_np = np.reshape([cam_00_to_world_dict[frame_id] for frame_id in range(seq_id, seq_id+64)], (-1, 4 , 4)) #[N, 4, 4]
        cam_01_hwf = np.array((cam_01_K[0,2], cam_01_K[1,2], cam_01_K[0,0])) # [3,]
        cam_01_hwf = np.broadcast_to(cam_01_hwf[:, None], cam_01_T_np.shape[:1] + (3,1)) #[N, 3, 1]
        cam_01_Tk_np = np.concatenate((cam_01_T_np[:, :3, :], cam_01_hwf), axis=2) #[N, 3, 5]
        # np.save(f'{sequence_name}_{seq_id}_cam_01_T_np', cam_01_T_np.reshape(-1, 3*5))
        #TODO: cam02 and cam03
    ...
    def load_cameras(self, camera_name, sequence_name, frame_ids):
        ...
        if camera_name == "cam_00":
            ...
            for frame_id in frame_ids:
                if frame_id in cam_00_T_dict:
                    Ts.append(cam_00_T_dict[frame_id])
                    tmp = cam_00_T_dict[frame_id]
                else:
                    Ts.append(tmp)
            ...
            for frame_id in frame_ids:
                if frame_id in cam_01_T_dict:
                    Ts.append(cam_01_T_dict[frame_id])
                    tmp = cam_01_T_dict[frame_id]
                else:
                    Ts.append(tmp)
...
def main():
    # Load cameras.
    k3 = KITTI360Loader(kitti_360_root=Path("data") / "kitti360/KITTI-360")
